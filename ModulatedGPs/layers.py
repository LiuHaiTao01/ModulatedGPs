...
class SVHGP_Layer(SVGP_Layer):
    def __init__(self, kern, kern_g, Z, Z_g, mu0_g, num_inducing, num_inducing_g, num_outputs, mean_function=None, white=True):
        SVGP_Layer.__init__(self, kern, Z, num_inducing, num_outputs, mean_function, white)
        self.kern_g = kern_g
        self.num_inducing_g = num_inducing_g
        self.q_diag_g = False
        self.mu0_g = Param(mu0_g, name="mu0_g")()
        Um_g = np.zeros((self.num_inducing_g, self.num_outputs))
        Us_sqrt_g = np.ones((self.num_inducing_g, self.num_outputs)) if self.q_diag_g else np.array([np.eye(self.num_inducing_g) for _ in range(self.num_outputs)])
        with tf.name_scope("inducing"):
            self.Z_g = Param(Z_g, name="z_g")()
            self.Um_g = Param(Um_g, name="u_g")()
            if self.q_diag_g:
                self.Us_sqrt_g = Param(Us_sqrt_g, transforms.positive, name="u_variance_g")()
            else:
                self.Us_sqrt_g = Param(Us_sqrt_g, transforms.LowerTriangular(self.num_inducing_g,self.num_outputs),name="u_variance_g")()

        self.Ku_g = self.kern_g.Ksymm(self.Z_g) + tf.eye(tf.shape(self.Z_g)[0],dtype=self.Z_g.dtype)*settings.numerics.jitter_level
        self.Lu_g = tf.cholesky(self.Ku_g)

    def conditional_ND_g(self, X, full_cov=False):
        Kuf_g = self.kern_g.K(self.Z_g, X)
        Kff_g = self.kern_g.Ksymm(X) if full_cov else self.kern_g.Kdiag(X)
        mean_g, var_g = self.base_conditional(Kmn=Kuf_g, Kmm=self.Ku_g, Knn=Kff_g,
                                              f=self.Um_g,
                                              full_cov=full_cov, q_sqrt=self.Us_sqrt_g, white=self.white)
        return mean_g + self.mu0_g, var_g

    def KL_g(self):
        Ku_g = None if self.white else self.Ku_g
        return gauss_kl(self.Um_g, self.Us_sqrt_g, Ku_g)

    def conditional_SND_g(self, X, full_cov=False):
        S, N, D = tf.shape(X)[0], tf.shape(X)[1], tf.shape(X)[2]

        f = lambda X: self.conditional_ND_g(X, full_cov=full_cov)
        mean_g, var_g = tf.map_fn(f,X, dtype=(settings.float_type,settings.float_type))

        if full_cov is True:
            return tf.reshape(tf.stack(mean_g),[S,N,self.num_outputs]), tf.reshape(tf.stack(var_g),[S,N,N,self.num_outputs])
        else:
            return [tf.reshape(m, [S, N, self.num_outputs]) for m in [mean_g, var_g]]

    def sample_from_conditional(self, X, z=None, full_cov=False):
        mean, var = self.conditional_SND(X, full_cov=full_cov) # S,N,D
        mean_g, var_g = self.conditional_SND_g(X, full_cov=full_cov) # S,N,D
        return None, None, mean, var, mean_g, var_g

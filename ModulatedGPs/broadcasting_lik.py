...
class BroadcastingLikelihood_HGP(BroadcastingLikelihood):
    def __init__(self, likelihood):
        BroadcastingLikelihood.__init__(self, likelihood)

    def variational_expectations(self, Fmu, Fvar, Fmu_g, Fvar_g, Y):
        f = lambda vars_SND, vars_ND: self.likelihood.variational_expectations(vars_SND[0],
                                                                                vars_SND[1],
                                                                                vars_SND[2],
                                                                                vars_SND[3],
                                                                                vars_ND[0])
        return self._broadcast(f,[Fmu, Fvar, Fmu_g, Fvar_g], [Y])

    def predict_mean_and_var(self, Fmu, Fvar, Fmu_g, Fvar_g):
         f = lambda vars_SND, vars_ND: self.likelihood.predict_mean_and_var(vars_SND[0],
                                                                             vars_SND[1],
                                                                             vars_SND[2],
                                                                             vars_SND[3])
         return self._broadcast(f,[Fmu, Fvar, Fmu_g, Fvar_g], [])